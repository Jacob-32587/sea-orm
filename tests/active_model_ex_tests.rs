...
#[sea_orm_macros::test]
async fn test_has_one_replace_and_delete() -> Result<(), DbErr> {
    use common::blogger::*;

    let ctx = TestContext::new("test_has_one_replace_and_delete").await;
    let db = &ctx.db;
...
#[sea_orm_macros::test]
async fn test_has_many_mutate() -> Result<(), DbErr> {
    use common::blogger::*;

    let ctx = TestContext::new("test_has_many_mutate").await;
    let db = &ctx.db;
...
#[sea_orm_macros::test]
async fn test_has_many_self_mutate() -> Result<(), DbErr> {
    use common::film_store::*;

    let ctx = TestContext::new("test_has_many_self_mutate").await;
    let db = &ctx.db;
    ...
    staff = staff.delete_existing_manage(&rick_pk).save(db).await?;
...
#[sea_orm_macros::test]
async fn test_belongs_to_duplicate_target() -> Result<(), DbErr> {
    use common::blogger::*;

    let ctx = TestContext::new("test_belongs_to_duplicate_target").await;
    let db = &ctx.db;

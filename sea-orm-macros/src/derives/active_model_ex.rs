    ...
    fn expand_compound(&self, compound_type: &CompoundType) -> syn::Result<TokenStream> {
        ...
        match compound_type.kind {
            ...
            CompoundKind::HasMany => {
                let plural_name = pluralizer::pluralize(&field_ident.to_string(), 1, false);
                let setter = format_ident!("add_{}", plural_name);
                let remove = format_ident!("delete_{}", plural_name);
                let remove_existing = format_ident!("delete_existing_{}", plural_name);
                ...
                Ok(quote! {
                    ...
                    #[doc = " Generated by sea-orm-macros"]
                    pub fn #remove(mut self, v: impl Into<#active_model_type>) -> Self {
                        self.#field_ident.push_delete(v.into());
                        self
                    }

                    #[doc = " Generated by sea-orm-macros"]
                    pub fn #remove_existing(mut self, v: impl std::borrow::Borrow<sea_orm::sea_query::ValueTuple>) -> Self {
                        self.#field_ident.push_delete_existing(v.borrow());
                        self
                    }
                })
            }
        ...
        let has_many_action = quote! {
            match #field_binding {
                ...
                ActiveHasMany::Mutate(ref mut mutations) => {
                    ...
                    for item in deletes {
                        let mut item: self::ActiveModel = item.into();
                        #delete_associated_model
                    }
                },
                _ => ()
